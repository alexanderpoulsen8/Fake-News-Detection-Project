...
_FILEPATH = data_dir / 'tf_idf' / 'big_doc_freq_vector.csv'
_OUTPUT_VOCAB_PATH = data_dir / 'tf_idf' / 'big_pruned_vocabulary.csv'
_OUTPUT_IDF_PATH = data_dir / 'tf_idf' / 'big_pruned_doc_freq_vector.csv'
...
def prune(
    ...
    print(df['term'].isin(['<NUM>', '<EMAIL>', '<URL>', '<DATE>']).astype(int).sum())
    print(df[df['doc_freq'] / N > max_df])
